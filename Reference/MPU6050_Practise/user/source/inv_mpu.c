...
#define i2c_write   MPU6xxx_Write_Len
#define i2c_read    MPU6xxx_Read_Len
...
//}
#define log_i 	printf	//printf log
#define log_e  	printf	
...
int mpu_init(void)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    delay_ms(100);
myPrintf("Reset device ok");
    /* Wake up chip. */
    data[0] = 0x00;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
myPrintf("Wake up chip ok");
#if defined MPU6050 /////////////////////////////////////////////////////////////////////////////////////////////
    /* Check product revision. */
    if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, data)){
			myPrintf(" Check product revision. fail");
        return -1;
		}
...
void mpu_EXTI_Configuration(void)
{
	...
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOB,GPIO_PinSource5);//选定管脚B5为外部中断源
	
	EXTI_InitStructure.EXTI_Line = EXTI_Line5;//选择外部中断线
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;//选择模式为中断
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;//选择下降沿触发
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;//使能中断线
	
	EXTI_DeInit();//初始化相关寄存器为缺省值
	
	EXTI_Init(&EXTI_InitStructure);//按结构体初始化
	
	EXTI_ClearITPendingBit(EXTI_Line5);//清除中断请求	
		
	NVIC_InitStructure.NVIC_IRQChannel = EXTI9_5_IRQn;//选择中断通道
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;//设置主优先级
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;//设置从优先级
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;//使能中断通道
	
	NVIC_Init(&NVIC_InitStructure);//按结构体初始化
